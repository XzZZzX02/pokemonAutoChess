...
class Simulation extends schema_1.Schema {
    ...
    applyWeatherEffects(pokemon) {
        if (this.weather === Weather_1.Weather.WINDY) {
            const nbFloatStones = pokemon.player
                ? (0, array_1.count)(pokemon.player.items, Item_1.Item.FLOAT_STONE)
                : 0;
            pokemon.addDodgeChance((pokemon.types.has(Synergy_1.Synergy.FLYING) ? 0.2 : 0.1) + nbFloatStones * 0.05, pokemon, 0, false);
        }
        else if (this.weather === Weather_1.Weather.NIGHT) {
            const nbBlackAugurite = pokemon.player
                ? (0, array_1.count)(pokemon.player.items, Item_1.Item.BLACK_AUGURITE)
                : 0;
            pokemon.addCritChance(10 + 5 * nbBlackAugurite, pokemon, 0, false);
        }
        else if (this.weather === Weather_1.Weather.MISTY) {
            const nbMistStones = pokemon.player
                ? (0, array_1.count)(pokemon.player.items, Item_1.Item.MIST_STONE)
                : 0;
            if (nbMistStones > 0) {
    ...
    applyPostEffects() {
        for (const team of [this.blueTeam, this.redTeam]) {
            ...
            const effects = team === this.blueTeam ? this.blueEffects : this.redEffects;
            if ([
            ...
            team.forEach((pokemon) => {
                if (pokemon.items.has(Item_1.Item.ROTOM_PHONE)) {
                    const teamIndex = team === this.blueTeam ? 0 : 1;
                    const rotomDrone = pokemon_factory_1.default.createPokemonFromName(Pokemon_1.Pkm.ROTOM_DRONE, pokemon.player);
